...
function createGui() {
  ...
  gui.addColor(guiData, "BackgroundColor").onChange(val => {
    scene.background = new THREE.Color(val);
  });

  if (modelList) {
    gui
      .add(guiData, "currentURL", modelList)
      .name("Model")
      .onChange(update);
  } else {
    gui
      .add(guiData, "currentURL", {
        "Loading...": "models/Mjolnir.obj",
      })
      .name("Loading ...")
      .onChange(update);
  }
  ...
  gui
    .add(guiData, "material", ["Basic", "Wireframe", "Glossy"])
...
function changeMaterial() {
  ...
  if (guiData.material == "Glossy") {
    ...
    props
      .addColor(phongData, "MeshColor")
      .onChange(val => (material.color = new THREE.Color(val)));
    props
      .addColor(phongData, "Specular")
      .onChange(val => (material.specular = new THREE.Color(val)));
    props
      .add(phongData, "Shininess", 0, 100)
      .onChange(val => (material.shininess = val));
    props.add(phongData, "FlatShading").onChange(val => {
